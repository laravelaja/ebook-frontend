...
export const EbookRead = () => {
  ...
  // Detect if this is a PDF-based ebook (pages contain <img> with pdf-page-image class)
  ...
  // PDF Page Renderer - clean full-page image display
  const renderPdfPage = (content: PageContent, index: number) => (
    <div
      key={index}
      className="relative w-full h-full flex items-center justify-center rounded-md overflow-hidden bg-white select-none border border-[#ebdcb9]"
    >
      <div 
        className="w-full h-full flex items-center justify-center [&>img]:w-full [&>img]:h-full [&>img]:object-contain"
        dangerouslySetInnerHTML={{ __html: content.content || '' }}
      />
    </div>
  );

  // Text Page Renderer - original book-style display
  const renderTextPage = (content: PageContent, index: number) => (
    <div
      key={index}
  ...
  const renderedPages = pagesContent.map((content, index) => 
    isPdfEbook ? renderPdfPage(content, index) : renderTextPage(content, index)
  );
  ...
  return (
    <div className="min-h-full w-full flex flex-col bg-gradient-to-br from-[#f8f3e8] via-[#eedfb8] to-[#e4d3a2]">
      ...
      {isPdfEbook ? (
        ...
        /* Text Mode - Original book-style display */
